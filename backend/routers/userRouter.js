...
userRouter.post(
    ...
    expressAsyncHandler(async (req, res) => {
        ...
        const response = await pool.query(`SELECT * FROM bs_usuario WHERE email = '${email}' `);
...
userRouter.get(
    ...
    expressAsyncHandler(async (req, res) => {
        const { id } = req.params;
        const response = await pool.query('SELECT * FROM bs_usuario WHERE cod_usuario = $1', [id]);
        if (response) {
...
userRouter.put(
    ...
    expressAsyncHandler(async (req, res) => {
        ...
        const password = bcrypt.hashSync(req.body.password, 8)
        const response = await pool.query('SELECT * FROM bs_usuario WHERE cod_usuario = $1', [userId]);
        const user = response.rows[0];
        if (user) {
            if (req.body.password) {
                user.nombre = req.body.nombre || user.nombre;
                user.email = req.body.email || user.email;
                const usuarioActualizado = await pool.query('UPDATE bs_usuario SET nombre = $1, email = $2, password = $3 WHERE cod_usuario = $4'
    ...
    expressAsyncHandler(async (req, res) => {
        ...
        if (user) {
            if (user.email === 'marceloalarcon29@gmail.com') {
                res.status(400).send({ message: 'No se puede Eliminar a este Administrador' });
                return;
            }
            const response2 = await pool.query(`DELETE FROM bs_usuario WHERE cod_usuario =$1`, [id]);
...
userRouter.put(
    ...
    expressAsyncHandler(async (req, res) => {
        ...
        const response = await pool.query(`SELECT * FROM bs_usuario WHERE cod_usuario = $1`, [cod_usuario]);
        const user = response.rows[0];
        if (user) {
            user.nombre = req.body.nombre || user.nombre;
            user.email = req.body.email || user.email;
            user.is_admin = Boolean(req.body.is_admin);
            const response2 = await pool.query(`UPDATE bs_usuario
            SET is_admin=$1, nombre=$2, email=$3
            WHERE cod_usuario =$4`, [isAdmin, nombre, email, cod_usuario])
